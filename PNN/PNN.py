...
class PNN:
    ...
    def initialize_network(self,ins, hiddens, outs):
    ...
    def PNNFit(self,net,epochs,train_fold_features,train_fold_labels,features_no,n_outputs,labelvalue,lrate,hn,b):
        ...
        for epoch in range(epochs):
            for i, row in enumerate((train_fold_features)):
                ...
                outputs ,statelayer = self.forward_propagation(net, xxx1,trainfoldexpected, n_outputs,labelvalue,b,statelayer)
                self.back_propagation(net, features_no, trainfoldexpected, outputs, n_outputs,labelvalue,b)
    ...
    def CrossValidationAvg(self,kfold,foldcounter,foldindex,X_train,y_train,featuresno, noofhidden, labelno,labelvalue,lrate,bbs,epochs,bestvector):
        net = self.initialize_network(featuresno, noofhidden, labelno)
        avr_res=0
        tot_etau=0
        for idx_train, idx_test in kfold.split(X_train):      
            ...
            net,error=self.PNNFit(net,epochs,X_train[idx_train,:],trainlabel,featuresno,labelno,labelvalue,lrate,noofhidden,bbs)
            # self.print_network(net)
            iterationoutput=self.predict(net,X_train[idx_test,:],testlabel,  labelno,labelvalue,bbs,noofhidden)
            # self.print_network(net)
    ...
    def training(self,epochs, X,y, featuresno, labelno,labelvalue,lrate,hn,scale):
        ...
        # iterationoutput=predict(bestnet,X_test_norm, y_test, labelno,labelvalue,bestvector[4],hn1)
        # print('Final Prediction=%f',iterationoutput)
        print(">>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<")
        return bestnet,avresult


    def predict(self,net1,test_fold_features,test_fold_labels, n_outputs,labelvalue,bx,hn):
        iterationoutput=np.array([])
        statelayer=list()
        for i, row in enumerate((test_fold_features)): 
            xxx1=np.array(list(row))    
            testfoldlabels=list(test_fold_labels[i])
            predicted,statelayer= self.forward_propagation(net1, row,testfoldlabels, n_outputs,labelvalue,bx,statelayer)
 ...
 ###############################################################
# pnn = PNN()
# train_error = pnn.loadData1(filename='C:\\Github\\PNN\\Data\\LRData\\iris.txt', featuresno=4,labelno=3,labelvalue=3,iteration=100,lrate=0.05,hn=100) 
